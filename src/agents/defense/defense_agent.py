"Defense Attorney Agent - 2025 LangChain Pattern"
import json
...
from src.prompts.defense_prompts import DEFENSE_SYSTEM_PROMPT, DEFENSE_HUMAN_TEMPLATE
...
class DefenseAgent:
    """
    변호사 에이전트 - 2025 LangChain 패턴 (CourtSimulationState 기반)

    LLM 선택:
    - provider: "openai" 또는 "upstage" (기본값: settings.default_llm_provider)
    - temperature: 0.7 (창의적이고 설득력 있는 변론을 위해)
    """
    ...
    def __init__(self, provider: Optional[Literal["openai", "upstage"]] = None):
        ...
        self.prompt = ChatPromptTemplate.from_messages(
            [
                ...
                ("human", DEFENSE_HUMAN_TEMPLATE),
            ]
        )

        self.chain = self.prompt | self.llm

        logger.info(f"DefenseAgent initialized with {self.provider.upper()} LLM")
    ...
    async def generate_argument(
        ...
        Returns:
            업데이트될 State의 딕셔너리 (주로 messages 필드)
        """
        # --- 1. State에서 필요한 정보 추출 ---
        ...
        # --- 4. LangSmith 트레이싱 설정 ---
        tracer = create_agent_tracer("defense", state)
        langsmith_config = {
        ...
        try:
            ...
            # 모든 재시도 실패 - 폴백
            logger.error(f"Defense JSON parsing failed after retries: {e}")
            ...
            agent_output_dict = create_fallback_agent_output(
                role=Role.DEFENSE,
                error_message="변호사의 변론을 생성하는 중 오류가 발생했습니다. 잠시 후 다시 시도해주세요."
            )

        # --- 6. State 업데이트용 딕셔너리 반환 ---
        return {
